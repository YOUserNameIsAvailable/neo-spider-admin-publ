import { Dispatch, FC, KeyboardEvent, SetStateAction, cloneElement, useEffect, useState } from "react";
...
export const UserManagementRoleModal: FC<{ setShowRoleModal: Dispatch<SetStateAction<boolean>>; userId: string }> = ({
  ...
  const getDetail = async () => {
    const detailJson = await fetch("/api/spider/user-management/menuList", {
      method: "POST",
      headers: {
    ...
    const mdofiy = detailResult?.body?.usermenulist || [];
  ...
  const updateDetail = async (e: any) => {
    const updateJson = await fetch("/api/spider/user-management/update", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
